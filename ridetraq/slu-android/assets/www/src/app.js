...
RTApp.clearAndReload = function() {
  ...
  var hash = h.substring(h.indexOf("#"), h.length);
  console.log("set hash: " + hash);
  LocalStorageManager.set(RTApp.C_LOCALSTORAGE_RELOAD_PAGE, {url: hash});
...
RTApp.C_LOCALSTORAGE_RELOAD_PAGE 				 = "C_LOCALSTORAGE_RELOAD_PAGE__________________";
// config 
RTApp.cfg = {};
...
//	  this.route(/^map\/(\d+)-(.*)?$/, "map");
//	  this.route(/^map\/(\d+)\/(\d+)-(.*)?$/, "map");
...
RTApp.views.SelectRouteView = Backbone.View.extend({
  ...
  goToPdf: function(e) {
   ...
   LocalStorageManager.set("url_to_go_back", {url: window.location.href });
